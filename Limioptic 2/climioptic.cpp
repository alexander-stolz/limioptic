...
void CLimioptic::ApplyBeamProfile(double *p)
{
   ...
   for (ip=0;ip<pnum;ip++) 
   {
	...
	datei << pnum*particlesize/elesize;	//transmission
	datei <<" ";
	datei << sqrt(xplus/pnum);		//strahlbreite x
	datei <<" ";
	datei <<sqrt(aplus/pnum);		//divergenz a
	datei <<" ";
	datei <<sqrt(xplus/pnum)*sqrt(aplus/pnum); //emittanz x
	datei <<" ";
	datei <<sqrt(yplus/pnum);	//strahlbreite y
	datei <<" ";
	datei <<sqrt(bplus/pnum);  //divergenz b
	datei <<" ";
...
void CLimioptic::ApplySlit(double *p,double x,double dx,double y,double dy)
{
   ...
   for (imat=0;imat<nmat;imat++) {
	  for (ip=0;ip<pnum;ip++) {
		if ((p[i+0-elesize]<=x+dx/2.) && (p[i+0-elesize]>=x-dx/2.) && (p[i+2-elesize]<=y+dy/2.) && (p[i+2-elesize]>=y-dy/2.))	
		{
			p0 = p[i+0-elesize];
			p1 = p[i+1-elesize];
			p2 = p[i+2-elesize];
   ...
   std::cout<<"transmission (slit) =\t"<<(double)durch/(durch+nichtdurch)<< "\t@ " << p[i+6-particlesize] <<" m\n";
   ...
   double r0,double beta0, double korrektur)
{
   ...
   for (imat = 0; imat < nmat; imat++) 
   {
	  for (ip = 0; ip < pnum; ip++) 
	  {
		 ...
		 if (imat == 0) { p[i+7] = p[i+7] + 1.; } // Index des ionenoptischen Elements raufzaehlen
   ...
   for (imat=0;imat<nmat;imat++) {
	  for (ip=0;ip<pnum;ip++) {
		 p0 = p[i+0-elesize] * cos(alpha) + 
			 ...
			 (p[i+5-elesize] + korrektur) * r * (1. - cos(alpha)) / 2; // Alex: angepasst
		 p1 = p[i+0-elesize] * (-sin(alpha) / r) + 
			  p[i+1-elesize] * cos(alpha) +
			  p[i+4-elesize] * sin(alpha) / 2 + 
		     (p[i+5-elesize] + korrektur) * sin(alpha) / 2;  // Alex: angepasst
		 p2 = p[i+2-elesize] + 
