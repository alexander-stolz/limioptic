...
void CLimioptic::AddAperture(double d, int output)
{
    vector<double> aperture;
    aperture.clear();
    aperture.push_back(29.0);
    aperture.push_back(1.);
    aperture.push_back(d);
    aperture.push_back(output);
    beamline.push_back(aperture);
}
...
void CLimioptic::CalculateTrajectories()
{
    ...
    while (n < nelements)    // Solange noch ionenoptische Elemente uebrig sind ..
    {
        ...
        case 9:
            ...
            break;
        case 29:
            ApplyAperture(&trajectories.front() + itraj,
                  beamline[ibeamline][2], beamline[ibeamline][3]);
            break;
        case 10:
...
void CLimioptic::ApplyAperture(double *p, double d, int output)
{
    int pnum, imat, i, j, elesize, ip;
    double p0, p1, p2, p3, p4, p5;
    int nmat = 1, durch = 0, nichtdurch = 0;
